...
<br>예를 들어 연구실에 총 4마리의 폰켓몬이 있고, 각 폰켓몬의 종류 번호가 [3번, 1번, 2번, 3번]이라면 이는 3번 폰켓몬 두 마리, 1번 폰켓몬 한 마리, 2번 폰켓몬 한 마리가 있음을 나타냅니다. 
<br>
...
function solution(nums) {
  var answer = 0;
  let set = Array.from(new Set(nums)).length;
  // 중복 제거 후 길이가 가져갈 수 있는 수보다 길거나 같다면 최대로 가져갈 수 있음
  if (set >= nums.length / 2) {
    answer = nums.length / 2;
  } else {
    // 중복이 없는 만큼만 다양한 종류를 가져갈 수 있음
    answer = set;
  }
